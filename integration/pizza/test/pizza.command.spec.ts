...
export const PizzaSuite = suite<{ commandInstance: TestingModule }>('Pizza Command');
PizzaSuite.before(async (context) => {
  context.commandInstance = await CommandTestFactory.createTestingCommand({
    imports: [PizzaModule],
  }).compile();
});
PizzaSuite('each option should be set-able from inquirer', async ({ commandInstance }) => {
  CommandTestFactory.setAnswers(['p', true, '9999999999', 'Large', 42, 'Coke', 'Nope, all good!']);
